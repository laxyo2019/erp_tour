				...
				<div class="tile-body">
					<div >
						...
						<tr>
							...
							<td><a href="{{route('tour-bill-upgradation.show',$datas->id)}}" target="_blank"><i class="fa fa-eye btn btn-primary" ></i></a>
							</td>
							<td><center>
                           ...
                           <center>
							...
							@if($datas->manager_status == 0)
								<a href="{{route('tour-bill-upgradation.edit',$datas->id)}}" class="btn btn-sm btn-outline-warning"><i class="fa fa-edit"></i> Edit</a>
				             	 <form method="post" action="{{ route('tour-bill-upgradation.destroy',$datas->id) }}">
		                       		 @csrf
		                       		 @method('DELETE')
				                  	{{-- Delete button --}}
