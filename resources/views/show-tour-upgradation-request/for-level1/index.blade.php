...
<main class="app-content">
	<div class="app-title">
		 <div>
          <h1><i class="app-menu__icon fa fa-comments-o"></i> Show Tour Upgradation </h1>
         </div>
		<div>
          {{-- Message show --}}
			<p>
		...
		<div class="tile">
			<div class="tile-body">
				<div >
					...
					<td>
                		@if($datas->requested_role == 'tour_lavel2')
                			<span> - </span>
                		@else
						 <span style="
							@if($datas->level2_status == 0) color:#ff9a00 
								@elseif($datas->level2_status == 1) color:green 
								@elseif($datas->level2_status == 2) color:#ff0000 @endif; font-weight: bold">
					...
					{{-- end For Accountant paid or not paid ammount --}}
						 ...
						 @if( $datas->level2_status == 0 && $roleName == 'tour_superadmin')

							<form action="{{route('approved-admin-upgradation-request')}}" method="POST">
							@csrf
