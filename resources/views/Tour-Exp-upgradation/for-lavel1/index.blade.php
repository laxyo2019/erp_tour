...
<a class="btn btn-info btn-lg" href="{{route('tour-admin-upgradation.create')}}"  ><i class="fa fa-send">New Request</i></a>
				...
				<div class="tile-body">
					<div >
						...
						<thead>
							<tr>
								<th>S.No</th>
								<th>Tour Upgradaton, From</th>
								<th>Tour Upgradaton, To</th>
						...
						<tr>
							...
							<td><a href="{{route('tour-admin-upgradation.show',$datas->id)}}" target="_blank"><i class="fa fa-eye btn btn-primary" ></i></a>
							</td>
							...
							<td>
								@if($datas->level2_status == 0)
										<a href="{{route('tour-admin-upgradation.edit',$datas->id)}}" class="btn btn-sm btn-outline-warning"><i class="fa fa-edit"></i> Edit</a>
						              	<form method="post" action="{{ route('tour-admin-upgradation.destroy',$datas->id) }}">
				                        @csrf
				                        @method('DELETE')
						                  {{-- Delete button --}}
								...
								@endif
									@if($datas->level2_status == 1 )
										...
										<?php
												...
												if($cnt ==1){
										?>
											<form method="post" id="formID_{{$datas->user_id}}" action="{{url('tour-upgradation-level-1-bill')}}">
						                        @csrf
						                        <input type="hidden" name="id" value="{{$datas->id}}">
												<button class="btn btn-info fa fa-reply">Apply T.A.Bill3 </button>
											</form>
										<?php } } elseif(strtotime($datas->time_from) == strtotime($sortDate) && $TABillCounts->ta_no != $datas->id) { ?>
											<form method="post" action="{{url('tour-upgradation-level-1-bill')}}">
						                        @csrf
						                        <input type="hidden" name="id" value="{{$datas->id}}">
												<button class="btn btn-info fa fa-reply">Apply T.A.Bill0 </button>
												{{-- <button class="btn btn-info fa fa-reply" disabled="true">Already Apply</button> --}}
											</form>
										<?php } } } if($count==0){
											//print_r($count);
										...
										?>
											<form method="post" action="{{url('tour-upgradation-level-1-bill')}}">
						                        @csrf
						                        <input type="hidden" name="id" value="{{$datas->id}}">
												<button class="btn btn-info fa fa-reply">Apply T.A.Bill1 </button>
											</form>
										<?php }else{ ?>
											<button class="btn btn-info fa fa-reply" disabled="true">Apply previous Bill</button>
										<?php } }  ?>
									...
									@elseif($datas->accountant_status == 0 && $datas->level1_status == 1)
										...
										<form method="post" action="{{url('tour-upgradation-level-1-bill')}}">
					                        @csrf
					                        <input type="hidden" name="id" value="{{$datas->id}}">
											<button class="btn btn-info fa fa-reply" disabled="" >Apply T.A.Bill2 </button>
										</form>
