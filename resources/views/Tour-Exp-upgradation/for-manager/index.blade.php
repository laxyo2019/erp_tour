...
<a class="btn btn-info btn-lg" href="{{route('tour-manager-upgradation.create')}}"  ><i class="fa fa-send">New Request</i></a>
						...
						<tr>
							...
							<td>
								@if($datas->level1_status == 0)
										<a href="{{route('tour-manager-upgradation.edit',$datas->id)}}" class="btn btn-sm btn-outline-warning"><i class="fa fa-edit"></i> Edit</a>
						              	<form method="post" action="{{ route('tour-manager-upgradation.destroy',$datas->id) }}">
				                        @csrf
				                        @method('DELETE')
						                  {{-- Delete button --}}
									...
									@if($datas->level2_status == 1 )
										...
										<?php 
											...
											if(in_array($datas->id, $arr2)){ 
												...
												if($count < 2){
										?>
										<button class="btn btn-info fa fa-reply" disabled="true">Already Apply1</button>
										<?php }elseif($count > 1){ ?>
										...
										<?php
												...
												if($cnt ==1){
										?>
											<form method="POST" id="formID_{{$datas->user_id}}" action="{{route('tour-upgradation-manager-bill')}}">
						                        @csrf
										...
										<?php } } elseif(strtotime($datas->time_from) == strtotime($sortDate) && $TABillCounts->ta_no != $datas->id) { ?>
											<form method="post" action="{{url('tour-upgradation-manager-bill')}}">
						                        @csrf
						                        <input type="hidden" name="id" value="{{$datas->id}}">
