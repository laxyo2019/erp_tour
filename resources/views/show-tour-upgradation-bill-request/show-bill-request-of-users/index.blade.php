...
<div class="app-title">
   <div>
          <h1><i class="app-menu__icon fa fa-comments-o"></i>Users Tour Upgradation Amount Bill</h1>
   </div>
   <div>
      {{-- Message show --}}
      <p>
               ...
               <table class="table table-striped table-hover table-bordered table-responsive" id="sampleTable" >
                  ...
                  @foreach($data as $datas)
                     <tr>
                        ...
                        <td>
                           <a href="{{route('tour-bill-upgradation.show',$datas->id)}}" target="_blank"><i class="fa fa-eye btn btn-primary " ></i></a>
                        </td>
                        ...
                        <td>{{$datas->tour_upgrade_justification}}</td>
                         ...
                         <td>
                           <center>
                              ...
                              @elseif($datas->manager_status == 1){{$datas->acct_bill_vari_res}} 
                              @elseif($datas->manager_status == 2){{$datas->acct_bill_discard_res}} 
                              @endif
                              </span>
                           </center>
                       </td>
                  ...
                  {{-- end for paid unpaid amount to accountant --}}
                        ...
                        @if( $datas->manager_status == 0 && $roleName == 'tour_manager')
                           <form action="{{route('user-tour-upg-approve-bill-by-manager')}}" method="POST">
                              @csrf
                           ...
                           <form action="{{route('user-tour-upg-bill-verify')}}" method="POST">
                              @csrf
                              <button type="submit" class="btn btn-success fa fa-thumbs-up acct__bill_vari_res" bootbox >Varified
                              <input type="hidden" name="request_id" value="{{1}}">
                              <input type="hidden" name="id" value="{{$datas->id}}">
                           ...
                           @if($datas->accountant_status_varied_bill == 1  && $datas->level1_status == 0 && $roleName == 'tour_admin')
                           <form action="{{route('user-tour-upgr-approve-bill-by-level1')}}" method="POST">
                              @csrf
                           ...
                           @elseif( $datas->level1_status == 1 && $datas->accountant_status_varied_bill == 1 && $roleName == 'tour_admin')
                           @endif

                           @if(  $datas->level1_status == 1 && $datas->level2_status == 0  && $roleName == 'tour_superadmin')
                           <form action="{{route('user-tour-upgr-approve-bill-by-level2')}}" method="POST">
                              @csrf
                          ...
                          @if(  $datas->level1_status == 1 && $datas->level2_status == 1  &&  $datas->accountant_status == 0 && $roleName == 'tour_accountant' )

                          @if($datas->due_amount < 0)
                           <form action="{{route('user-tour-upg-approve-bill-by-accountant')}}" method="POST">
                              @csrf
                          ...
                          @endif
                           ...
                           {{-- @elseif( $$datas->level1_status == 1 && $datas->level2_status == 1 && $datas->accountant_status == 1)
                              <span style="color: green;"> <p>Amount Paid</p></span>
                            @elseif( $datas->level1_status == 1 && $datas->level2_status == 1 && $datas->accountant_status == 3)
                              <span style="color: green;"> <p>Amount Received</p></span>
                        
                           @elseif(  $datas->level1_status == 1 && $datas->level2_status == 1 && $datas->accountant_status == 2)
